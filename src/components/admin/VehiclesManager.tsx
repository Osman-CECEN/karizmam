...
import {
  ...
  parseVehicleStoragePathFromPublicUrl,
  turkishStorageErrorMessage,
  validateVehicleImageFile,
} from "@/lib/vehicles/vehicleImageStorage";
...
function rowToForm(row: VehicleRow): VehicleInput {
  return {
    title: row.title,
...
async function uploadVehicleImageClient(
  vehicleId: string,
  file: File
  ...
  const path = buildVehicleObjectPath(vehicleId, file);
  if (!path) {
...
async function removeStorageObjectIfInBucket(publicUrl: string): Promise<void> {
  const path = parseVehicleStoragePathFromPublicUrl(publicUrl);
  if (!path) return;
  const supabase = createClient();
  if (!supabase) return;
  const { error } = await supabase.storage.from("vehicles").remove([path]);
  if (error) {
    console.warn("[vehicles] Eski dosya silinemedi:", error.message);
  }
}

export function VehiclesManager({ initialRows, listError }: Props) {
  ...
  function validate(): string | null {
    if (!form.title.trim()) return "Model / başlık zorunludur.";
  ...
  function save() {
    ...
    if (dialog?.mode === "create" && !selectedFile) {
      setFormError("Yeni araç için bir görsel dosyası seçmelisiniz.");
      return;
    }
    ...
    startTransition(async () => {
      ...
      if (dialog?.mode === "create") {
        const res = await createVehicle({ ...form, image_url: null });
        if (!res.ok || !res.id) {
        ...
        if (selectedFile) {
          setUploading(true);
          const up = await uploadVehicleImageClient(newId, selectedFile);
          setUploading(false);
          if (!up.ok) {
            setFormError(up.error);
            await deleteVehicle(newId);
            return;
          }
          const upd = await updateVehicle(newId, {
            ...form,
            image_url: up.publicUrl,
          });
          if (!upd.ok) {
            setFormError(upd.error ?? "Görsel adresi kaydedilemedi.");
            await removeStorageObjectIfInBucket(up.publicUrl);
            await deleteVehicle(newId);
            return;
          }
        }
      ...
      if (dialog?.mode === "edit") {
        ...
        if (selectedFile) {
          setUploading(true);
          const up = await uploadVehicleImageClient(dialog.row.id, selectedFile);
          setUploading(false);
          if (!up.ok) {
            setFormError(up.error);
            return;
        ...
        const res = await updateVehicle(dialog.row.id, {
          ...form,
          image_url: nextImage,
        });
        if (!res.ok) {
        ...
        if (
          oldUrl &&
          parseVehicleStoragePathFromPublicUrl(oldUrl) &&
          oldUrl !== (nextImage ?? "")
        ) {
  ...
  function toggleActive(row: VehicleRow) {
    startTransition(async () => {
      const res = await setVehicleActive(row.id, !row.is_active);
      if (!res.ok) {
  ...
  return (
    <div>
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-2xl font-bold text-[#111111]">Araçlar</h1>
          <p className="mt-1 text-sm text-neutral-600">
            Ana sayfadaki araç vitrinini buradan yönetirsiniz. Görseller Supabase
            Storage (<code className="rounded bg-neutral-100 px-1">vehicles</code>)
            bucket’ına yüklenir.
          </p>
        </div>
        <button
      ...
      {initialRows.length === 0 && !listError ? (
        <p className="mt-8 rounded-xl border border-dashed border-neutral-300 bg-white px-4 py-10 text-center text-sm text-neutral-600">
          Henüz araç kaydı yok. “Yeni araç” ile ekleyebilirsiniz.
        </p>
      ) : (
              ...
              {initialRows.map((row) => (
                <tr key={row.id} className="border-b border-neutral-100">
                  <td className="px-3 py-2">
                    <div className="relative size-16 overflow-hidden rounded-lg bg-neutral-100">
                      {row.image_url?.trim() ? (
                        <Image
        ...
        >
          <div className="max-h-[90vh] w-full max-w-lg overflow-y-auto rounded-2xl bg-white p-6 shadow-xl">
            <h2 className="text-lg font-bold text-[#111111]">
              {dialog.mode === "create" ? "Yeni araç" : "Aracı düzenle"}
            </h2>

            <div className="mt-4 space-y-3">
              <div>
                <label className="text-sm font-medium">Model / başlık *</label>
                <input
              ...
              <div>
                ...
                <textarea
                  className="mt-1 min-h-[88px] w-full rounded-lg border border-neutral-300 px-3 py-2 text-sm"
              ...
              <div>
                <span className="text-sm font-medium" id="vehicle-photo-label">
                  Fotoğraf
                </span>
                <p
                  id="vehicle-photo-hint"
                  className="mt-0.5 text-xs text-neutral-500"
                >
                  JPEG, PNG veya WebP — en fazla 2 MB. Yol:{" "}
                ...
                <input
                  ...
                  aria-labelledby="vehicle-photo-label vehicle-photo-hint"
                  onChange={(e) => {
                ...
                <div className="mt-3 flex flex-col gap-3 sm:flex-row sm:items-start">
                  ...
                  <div className="flex flex-col flex-wrap gap-2">
                    <button
                      type="button"
                      disabled={busy || !browserSupabaseConfigured}
                      aria-controls="vehicle-photo-input"
                      aria-describedby="vehicle-photo-hint"
